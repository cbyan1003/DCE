...
from models.sd_model.utils import new_resize_image, HWC3
from copy import deepcopy
import einops
ImageFile.LOAD_TRUNCATED_IMAGES = True

class AbstractDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, name, split, data_root):
        # dataset parameters
        self.name = name
        self.root = data_root
        self.split = split
    ...
    def load_rgb(self, rgb_fn):
        rgb = Image.open(os.path.join(self.root, rgb_fn)).convert("RGB")
        img = deepcopy(rgb)
        img = new_resize_image(img, self.cfg.img_dim)
        return rgb, img

    def load_sd_feat(self, path):
        with open(os.path.join(self.root, path), "rb") as f:
            data_dict = pickle.load(f)
        return [torch.tensor(item) for item in data_dict]

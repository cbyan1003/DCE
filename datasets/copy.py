...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000):
    ...
    for i in range(len(dataset)):
        ...
        # update histogram
        counts = [torch.sum(neighb_mat.squeeze(0) < neighb_mat.squeeze(0).shape[0], dim=1).numpy() for neighb_mat in batched_input['neighbors']]
...
def collate_fn_DP(list_data, config, neighborhood_limits):
    ...
    dict_inputs = {
        ...
        'p2i_list': p2i_list,
        'i2p_list': i2p_list,
        'points': input_points, # cat在一起的两个体素化的点云 增加B
        'neighbors': input_neighbors, # 增加B
        'pools': input_pools,
        'upsamples': input_upsamples,
        'features': batched_features.float(),
        'stack_lengths': input_batches_len,

...
class VideoDataset(AbstractDataset):
    ...
    def __init__(self, cfg, root_path, data_dict, split):
        ...
        self.instances = self.dict_to_instances(self.data_dict, strided) # 建立图像对，0和20，间隔20张图
    ...
    def __getitem__(self, index):
        ...
        for i, id_i in enumerate(f_ids):
            rgb_ori, rgb = self.load_rgb(s_instance[id_i]["rgb_path"])
            # calculate crop offset to rescale K; should be the same for all images
            smaller_dim = rgb_ori.height
            crop_offset = (rgb_ori.width - rgb_ori.height) / 2
            ...
            P = torch.tensor(np.linalg.inv(P) @ P_rel).float() # np.linalg.inv(P) @ P_rel: 这部分将计算 P 的逆矩阵与 P_rel 矩阵的矩阵乘法
...
class VideoDataset_hybrid(AbstractDataset):
    ...
    def __init__(self, cfg, root_path, data_dict, split):
        name = cfg.name
        super(VideoDataset_hybrid, self).__init__(cfg, name, split, root_path)
        self.cfg = cfg
        self.split = split
        self.use_padding = cfg.use_padding
        self.num_downsample = cfg.num_downsample
    ...
    def __getitem__(self, index):
        ...
        for i, id_i in enumerate(f_ids):
            ...
            rgb_ori, rgb = self.load_rgb(s_instance[id_i]["rgb_path"])
            # TODO
            rgb_sd_feat = self.load_sd_feat(s_instance[id_i]["rgb_sd_path"])
            dep_sd_feat = self.load_sd_feat(s_instance[id_i]["dep_sd_path"])
            # calculate crop offset to rescale K; should be the same for all images
            smaller_dim = rgb_ori.height
            crop_offset = (rgb_ori.width - rgb_ori.height) / 2
            ...
            output["rgb_sd_" + str(i)] = rgb_sd_feat
            output["dep_sd_" + str(i)] = dep_sd_feat

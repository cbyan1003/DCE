...
def align(corres, P, Q, align_cfg, return_chamfer=False):
    ...
    corr_P_idx, corr_Q_idx, weights = corres
    ...
    Rt = teaser_rt(corr_P, corr_Q, weights)
...
def teaser_rt(corr_P, corr_Q, weights):
    """
    corr_P [B x N x 3]  : source point cloud
    corr_Q [B x N x 3]  : target point cloud
    Rt [B x 3 x 4]      : Estimated transformation from corr_P to corr_Q
    """
    corr_Pt = corr_P.permute(0,2,1)
    corr_Qt = corr_Q.permute(0,2,1)
    B, _, _ = corr_P.shape
    Rt = []
    for i in range(B):
        A = corr_Pt[i].cpu().numpy()
        B = corr_Qt[i].cpu().numpy()
        # weights = weights[i].cpu().numpy()
        NOISE_BOUND = 0.025
        teaser_solver = get_teaser_solver(NOISE_BOUND)
        teaser_solver.solve(A,B)
        solution = teaser_solver.getSolution()
        R_teaser = torch.tensor(solution.rotation).double().to("cuda")
        t_teaser = torch.tensor(solution.translation).double().to("cuda")
        Rt.append(torch.cat((R_teaser, t_teaser.view(3,1)), dim=1))
        # T_teaser = Rt2T(R_teaser,t_teaser)
    Rt = torch.stack(Rt, 0).float()
    return Rt

...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        # 额外的阴影点坐标巨大 s_pts.shape = [n_points, dim = 3]
        s_pts = torch.cat((s_pts, torch.zeros_like(s_pts[:1, :]) + 1e6), 0)

        # Get neighbor points [n_points, n_neighbors, dim] 
        # neighbor_inds是一个记录了每个点的邻居点的索引的矩阵，一个点记录29个邻居索引
        if torch.any(neighb_inds.flatten() >= s_pts.size(0)) or torch.any(neighb_inds.flatten() < 0): # GPU =1 neighb_inds 74228,29 s_pts 74229,3 GPU=2 neighb_inds 37114,29 37115,3
            print("neighb_inds中有越界索引")
            print(f"neighb_inds: {neighb_inds}")
            print(f"s_pts size: {s_pts.size(0)}")
            raise IndexError("neighb_inds contains out-of-bounds indices")

        neighbors = s_pts[neighb_inds, :]

        # Center every neighborhood 
        # 减法的作用是所有的邻居都减去中心点的坐标，作为相对于中心点的坐标系
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, x, points, neighbors, pools):
        ...
        if 'strided' in self.block_name:
            ...
            # collate_fn_DP
            # out = []
            # for i in range(points[0].size(0)):
            #     q_pts = points[self.layer_ind + 1][i].squeeze(0)
            #     s_pts = points[self.layer_ind][i].squeeze(0)
            #     neighb_inds = pools[self.layer_ind + 1][i].squeeze(0)
            #     out.append(self.KPConv(q_pts, s_pts, neighb_inds, x[i]).unsqueeze(0))
            # x = torch.vstack(out)
        else:
            # collate_fn_DDP
            ...
            # collate_fn_DP
            # for i in range(points[0].size(0)):
            #     q_pts = points[self.layer_ind][i].squeeze(0)
            #     s_pts = points[self.layer_ind][i].squeeze(0)
            #     neighb_inds = neighbors[self.layer_ind][i].squeeze(0)
            #     out.append(self.KPConv(q_pts, s_pts, neighb_inds, x[i]).unsqueeze(0))
            # x = torch.vstack(out)
        out = self.KPConv(q_pts, s_pts, neighb_inds, x)
        return self.leaky_relu(self.batch_norm(out))
...
class ResnetBottleneckBlock(nn.Module):
    ...
    def forward(self, features, points, neighbors, pools):
        if 'strided' in self.block_name:
            q_pts = points[self.layer_ind + 1]
            s_pts = points[self.layer_ind]
            neighb_inds = pools[self.layer_ind]
        ...
        #         # First downscaling mlp
        #         x = self.unary1(features[i].unsqueeze(0))

        #         # Convolution
        #         x = self.KPConv(q_pts, s_pts, neighb_inds, x.squeeze(0))
        #         x = self.leaky_relu(self.batch_norm_conv(x.unsqueeze(0)))
        ...
        #         # Shortcut
        #         if 'strided' in self.block_name:
        #             shortcut = max_pool(features[i], neighb_inds).unsqueeze(0)
        #         else:
        #             shortcut = features[i].unsqueeze(0)
        #         shortcut = self.unary_shortcut(shortcut)
                
        #         out_x.append(x)
        #         out_shortcut.append(shortcut)
        #     out_x = torch.vstack(out_x)
        #     out_shortcut = torch.vstack(out_shortcut)
        # else:
        #     out_x = []
        #     out_shortcut = []
        #     for i in range(points[0].size(0)): 
        ...
        # return self.leaky_relu(out_x + out_shortcut)
...
class NearestUpsampleBlock(nn.Module):
    ...
    def forward(self, x, upsamples_0):
        # collate_fn_DP
        # B, _, _ = x.shape
        # out = []
        # for i in range(B):
        #     out.append(closest_pool(x[i], upsamples_0[self.layer_ind - 1][i]).unsqueeze(0))
        # return torch.vstack(out)
    
        return closest_pool(x, upsamples_0[self.layer_ind - 1])

...
class PointsRenderer(nn.Module):
    ...
    def forward(self, points, features) -> torch.Tensor:
        ...
        idx, zbuf, dist_xy = rasterize_points(pointcloud, self.S, self.r, self.K) # S 图像的尺寸 r 光栅化的阈值 K  ； idx:是每个像素内点的索引，zbuf:是每个像素内点的深度，dist_xy:是每个像素内点的距离

        # Calculate PC coverage
        valid_pts = (idx >= 0).float() # 这里是在计算每个像素内点的有效性，如果idx>=0，那么就是有效的，否则就是无效的
        valid_ray = valid_pts[:, :, :, 0] # 每个像素的有效光线

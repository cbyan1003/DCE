...
def evaluate_3d_correspondances(xyz_0, xyz_1, K, Rt, img_dim):
    ...
    def xyz_to_camera(xyz):
        uvd = xyz.bmm(K.transpose(1, 2)) # 将 3D 点乘以相机的内参矩阵，得到齐次坐标, uvd[:, :, 0] 和 uvd[:, :, 1] 是图像平面的坐标，uvd[:, :, 2] 是深度（即 w 分量
        return uvd[:, :, :2] / uvd[:, :, 2:3] # 将齐次坐标转换为二维图像坐标（除以深度）

    xyz_all_0 = torch.cat((xyz_0, xyz_0in1), dim=1) # 将原始和转换后的 3D 点在第一个视角下拼接在一起
    xyz_all_1 = torch.cat((xyz_1in0, xyz_1), dim=1)
   
    uv_all_0 = xyz_to_camera(xyz_all_0) # 将拼接后的 3D 点投影到图像平面
    uv_all_1 = xyz_to_camera(xyz_all_1) 

    uv_0 = xyz_to_camera(xyz_0) # 将原始的 3D 点投影到图像平面
    uv_1 = xyz_to_camera(xyz_1)

    H, W = img_dim

    valid_dep = (xyz_all_0[:, :, 2] > 0) & (xyz_all_1[:, :, 2] > 0) # 检查深度是否大于 0
    valid_x_l = (uv_all_0[:, :, 0] > 0) & (uv_all_1[:, :, 0] > 0) # 检查 x 坐标是否在图像宽度范围内
    valid_x_h = (uv_all_0[:, :, 0] < W) & (uv_all_1[:, :, 0] < W)
    valid_y_l = (uv_all_0[:, :, 1] > 0) & (uv_all_1[:, :, 1] > 0) # 检查 y 坐标是否在图像高度范围内
    valid_y_h = (uv_all_0[:, :, 1] < H) & (uv_all_1[:, :, 1] < H)
    # valid_dep 检查深度是否大于 0。
    # valid_x_l 和 valid_x_h 检查 x 坐标是否在图像宽度范围内。
    # valid_y_l 和 valid_y_h 检查 y 坐标是否在图像高度范围内。
    
    valid = torch.stack((valid_dep, valid_x_l, valid_x_h, valid_y_l, valid_y_h), dim=2)

    valid = valid.all(dim=2).float() 
    valid_denom = valid.sum(dim=1).clamp(min=1) # 计算有效点的数量，并用 clamp(min=1) 确保最小值为 1，以避免除零错误
    valid_points = valid.mean(dim=1) # 计算有效点的平均值

    error_2d = (uv_all_0 - uv_all_1).norm(dim=2, p=2) # 计算 2D 坐标的欧几里得距离误差
    error_2d_mean = (torch.nan_to_num(error_2d, nan=0.0) * valid).sum(dim=1) / valid_denom # 计算有效点的 2D 坐标误差的均值

    error_3d = (xyz_all_0 - xyz_all_1).norm(dim=2, p=2) # 计算 3D 坐标的欧几里得距离误差
    ...
    # 代码还计算了在不同像素阈值下，2D 坐标误差在有效点中的比例
